   ...
   <header className="absolute top-2 w-full left-0 right-0 z-50 sm:w-full">
      ...
      <div className={`${active ? "py-0" : "py-1"}max-padd-container  transition-all duration-200 rounded-full px-6  ring-slate-900/5`}>
        <div className="flexBetween py-3">
            ...
            {!menuOpened ? (<MdMenu onClick={toggleMenu} className="xl:hidden cursor-pointer text-3xl text-[#FFF6DA] hover:text-secondary"/>) : (<MdClose onClick={toggleMenu} className="xl:hidden cursor-pointer text-3xl text-[#FFF6DA] hover:text-secondary" />)}
