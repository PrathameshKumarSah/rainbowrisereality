      ...
      <div className={`${active ? "py-0" : "py-1"}max-padd-container  transition-all duration-200 rounded-full px-6  ring-slate-900/5`}>
        <div className="flexBetween py-3">
            ...
            <div className="flexCenter gap-x-4 ">
                {/* Desktop */}
                <Navbar containerStyles={"hidden 2xl:flex gap-x-5 xl:gap-x-10 capitalize text-bolder  medium-15 ring-slate-900/10 rounded-full p-2"} />
                {/* Mobile */}
                <Navbar containerStyles={` mobile-navbar ${menuOpened ? "flex items-start flex-col gap-y-8 capitalize fixed top-20 right-8 p-12 bg-white rounded-3xl shadow-md w-64 medium-16 ring-1 ring-slate-900/5 transition-all duration=300 z-50" : "flex items-start flex-col gap-y-8 capitalize fixed top-20 -right-[100%] p-12 bg-white rounded-3xl shadow-md w-64 medium-16 ring-1 ring-slate-900/5 transition-all duration=300 z-50"}`} />
            </div>
            {/* button */}
